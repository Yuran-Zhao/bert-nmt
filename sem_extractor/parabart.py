...
class AttentionPooling(nn.Module):
    def __init__(self, config):
        ...
        self.config = config
        self.hidden_size = config.d_model
        self.q_transform = nn.Linear(self.hidden_size,
                                     self.hidden_size,
                                     bias=False)
        self.k_transform = nn.Linear(self.hidden_size,
                                     self.hidden_size,
                                     bias=False)
        self.dropout = 0.0
        # self.v_transform = nn.Linear(config.d_model, config.d_model, bias=False)
