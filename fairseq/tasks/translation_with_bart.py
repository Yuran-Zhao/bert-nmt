...
def load_langpair_dataset(data_path, split, src, src_dict, tgt, tgt_dict,
    ...
    for k in itertools.count():
        ...
        if split_exists(split_k, src, tgt, src, data_path):
            ...
            semprefix = os.path.join(data_path,
                                     '{}.sem.{}-{}.'.format(split_k, src, tgt))
        elif split_exists(split_k, tgt, src, src, data_path):
            prefix = os.path.join(data_path,
                                  '{}.{}-{}.'.format(split_k, tgt, src))
            semprefix = os.path.join(data_path,
                                     '{}.sem.{}-{}.'.format(split_k, tgt, src))
        else:
            if k > 0:
                break
            else:
    ...
    if len(src_datasets) == 1:
        ...
        srcsem_datasets = srcsem_datasets[0]
    else:
...
@register_task('translation_with_bart')
class TranslationWithBartTask(FairseqTask):
    """
    Translate from one (source, in this case, en) language to another (target) language with the help of Bart.
    ...
    def add_args(parser):
        ...
        parser.add_argument('--sem_extractor_path',
                            default='model.pt',
                            type=str)
        parser.add_argument('--encoder-ratio', default=1., type=float)
        parser.add_argument('--sem-extractor-ratio', default=1., type=float)
        parser.add_argument('--finetune-sem-extractor', action='store_true')
        ...
        parser.add_argument('--sem-extractor-gates',
                            default=[1, 1, 1, 1, 1, 1],
                            nargs='+',
                            type=int)
        parser.add_argument(
        ...
        parser.add_argument(
            '--encoder-sem-extractor-dropout',
            action='store_true',
        )
        parser.add_argument('--encoder-sem-extractor-dropout-ratio',
                            default=0.25,
                            type=float)
        parser.add_argument('--sem-extractor-output-layer',
                            default=-1,
                            type=int)
        parser.add_argument('--encoder-sem-extractor-mixup',
                            action='store_true')
        parser.add_argument('--decoder-no-sem-extractor', action='store_true')
    ...
    def __init__(self, args, src_dict, tgt_dict):
        ...
        self.sem_extractor_path = args.sem_extractor_path
    ...
    def load_dataset(self, split, epoch=0, combine=False, **kwargs):
        ...
        self.datasets[split] = load_langpair_dataset(
            ...
            sem_extractor_path=self.sem_extractor_path)

    def build_dataset_for_inference(self, src_tokens, src_lengths, srcbert,
                                    srcbert_sizes, berttokenizer):
